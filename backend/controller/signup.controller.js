...
async function userSignup(req, res) {
  ...
  if (!validatePassword(password)) {
    res.status(400).json({
      message:
        "Must use one uppercase, one lowercase, one number  and at least 8 char long",
    });
    return;
  }
...
async function adminSignup(req, res) {
  ...
  if (!validatePassword(password)) {
    res.status(400).json({
      message:
        "Password must use one uppercase, one lowercase, one number, one special char and at least 8 char long",
    });
    return;
  }
  if (!req.file) {
